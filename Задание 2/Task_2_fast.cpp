...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < iterations; i++){
        ...
        if (!((rank % 2 == 0) && (rank == size - 1))){
            if(rank % 2 == 0){
                MPI_Send(&steps[process_size - 1], 1, MPI_DOUBLE, rank + 1, 2, MPI_COMM_WORLD);
            }else{
                MPI_Recv(&data[0], 1, MPI_DOUBLE, rank - 1, 2, MPI_COMM_WORLD, &Status);
            }

            if(rank % 2 == 1){
                MPI_Send(&steps[0], 1, MPI_DOUBLE, rank - 1, 3, MPI_COMM_WORLD);
            }else{
                MPI_Recv(&data[1], 1, MPI_DOUBLE, rank + 1, 3, MPI_COMM_WORLD, &Status);
            }
        }

        if(!((rank % 2 == 1 && rank == size - 1) || (rank == 0))){
            if(rank % 2 == 1){
                MPI_Send(&steps[process_size - 1], 1, MPI_DOUBLE, rank + 1, 4, MPI_COMM_WORLD);
            }else{
                MPI_Recv(&data[0], 1, MPI_DOUBLE, rank - 1, 4, MPI_COMM_WORLD, &Status);
            }

            if(rank % 2 == 0){
                MPI_Send(&steps[0], 1, MPI_DOUBLE, rank - 1, 5, MPI_COMM_WORLD);
            }else{
                MPI_Recv(&data[1], 1, MPI_DOUBLE, rank + 1, 5, MPI_COMM_WORLD, &Status);
            }
        }
    ...
    if(rank == 0){
        ...
        for(int i = 1; i < size; i++){
            ...
            MPI_Recv(&process_answer[0], process_sizes[i], MPI_DOUBLE, i, 6, MPI_COMM_WORLD, &Status);
        ...
        MPI_Send(&steps[0], process_size, MPI_DOUBLE, 0, 6, MPI_COMM_WORLD);
    }
